...
**作成日**: 2026-04-03
**最終更新**: 2026-06-30（画像ギャラリー機能 + Vercel Cron keepalive を反映）
**ステータス**: Phase 1 + Phase 2 + Phase 4 完了 / Phase 3 (29-32, 37 done; 33 保留; 34-36 廃止; 38-39 pending) / 画像ギャラリー完了
...
さらに、テキスト導線（記事一覧）に対して「写真から記事に入る」導線として**画像ギャラリー**（公開 `/gallery` + 管理 `/admin/gallery`）を追加した。
...
### 3.6 写真ギャラリー (`/gallery`)
- **レンダリング**: force-dynamic (訪問のたびに並びをランダム化するため)
- **並び順**: ピン留め (is_featured) → シード付きランダム。訪問ごとにシードを生成し、初回ページと「もっと見る」で同じシードを共有 → ページングが安定 (重複・欠落なし)、リロードのたびに並びが変わる
- **データ取得**: `get_gallery_images(p_limit, p_offset, p_seed)` RPC で先頭ページ (GALLERY_PAGE_SIZE=30 + 1で hasMore 判定)
- **構成**: masonry レイアウト (CSS columns) の InfiniteImageGrid + 「もっと見る」(`GET /api/gallery?seed=`)。ライトボックスは無く、各タイルは `<Link>` で記事へ直行 (★Pick バッジ + ホバーオーバーレイ)
- **SEO**: ImageGallery JSON-LD + sitemap に登録

### 3.7 AI Chat (`/chat`) — (chat) ルートグループ
- **レンダリング**: force-dynamic
...
### 3.9 情報窓口フォーム (`/request/mini`, `/request/long`)
...
- **ギャラリー** — `gallery/_components/InfiniteImageGrid.tsx` (Client, masonry + もっと見る) / `gallery/_components/GalleryTile.tsx` (1枚タイル, `<Link>` 記事直行)
...
- ギャラリー管理: GalleryAdminClient (フィルタタブ + 件数バッジ) / GalleryAdminCard (表示 ON/OFF + ピン留めトグル, 楽観的更新)
...
| `/api/gallery` | GET | なし | ギャラリー追加読込 (`get_gallery_images` RPC ラップ、`offset`/`limit`/`seed` パラメータ) |
...
| エンドポイント | メソッド | 認証 | 説明 |
|--------------|---------|------|------|
| `/api/keepalive` | GET | `Authorization: Bearer ${CRON_SECRET}` | Supabase スリープ防止。`force-dynamic` + `categories` へ `head:true/count:'exact'` の軽量クエリで実 DB アクティビティを発生。成功 200 `{ok,count,timestamp}` / 認証失敗 401 / クエリ失敗 500 |

> `vercel.json` の `crons` で 1 日 1 回呼び出し (`schedule: "0 3 * * *"` = JST 12:00)。Hobby プランは 1日1回まで・Supabase は 7日無アクティビティで停止のため十分。Vercel Cron が `CRON_SECRET` を Bearer で自動付与する。
...
| **post_images** | 1203 | ギャラリー用 画像抽出テーブル (本文 + サムネ) |
...
### 7.11 post_images テーブル (ギャラリー)
記事本文 HTML + サムネから抽出した画像を保持。posts の公開状態/公開日時を非正規化 (`post_is_published` / `post_published_at`) し、posts 結合なしで部分インデックスを効かせる。抽出・同期はアプリ層 (`syncPostImages`) が担当し、DB トリガで HTML パースはしない。
...
| caption / alt | text nullable | figcaption / alt |
...
| post_is_published | boolean default false | 非正規化: 記事の公開状態 |
| post_published_at | timestamptz nullable | 非正規化: 記事の公開日時 (並び順用) |
| created_at / updated_at | timestamptz | |
| UNIQUE (post_id, image_url) | | 同一画像の重複排除 |
...
| ig_posts / post_images | post_id | posts.id | CASCADE |
...
- `idx_post_images_gallery_partial` — (is_featured DESC, post_published_at DESC, position ASC) WHERE post_is_published AND is_visible（部分 WHERE で公開＆表示行を絞る用。並びがランダム化された現在は ORDER BY のソート自体には使われないが、対象行のフィルタには有効）
...
全15テーブルで RLS 有効。`auth.role()` は `(select auth.role())` でサブクエリラップ済み。

### 公開テーブル (匿名 SELECT 許可)
- posts: `is_published = true` / categories: `is_active = true` / hashtags: 全件 / post_categories・post_hashtags: 全件 / reactions: 全件 (変更は RPC のみ) / ai_prompt_tags: `is_active = true` / ai_usage_limits: 全件 / article_knowledge: `is_active = true` / post_images: `post_is_published = true AND is_visible = true`

### 認証ユーザー限定 (匿名アクセス無し)
- ig_posts / ig_settings / mini_inquiries / long_inquiries: `(select auth.role()) = 'authenticated'` の ALL ポリシーのみ（匿名 SELECT/INSERT 無し）
- post_images: 上記公開 SELECT に加え、認証ユーザーには `(select auth.role()) = 'authenticated'` の ALL ポリシー（表示 ON/OFF・ピン留めの管理操作用）
...
increment_post_view_count / increment_reaction_count / decrement_reaction_count / search_posts / search_suggestions / match_articles / check_usage_limit / log_ai_usage / get_today_usage_stats / get_monthly_usage_stats / get_daily_usage_breakdown / get_top_queries / get_gallery_images / update_hashtag_count(trigger) / update_updated_at_column(trigger) / list_storage_objects

- `get_gallery_images(p_limit, p_offset, p_seed)` — STABLE SECURITY DEFINER。post_images ⨝ posts ⨝ 主カテゴリ (order_num 最小) を結合し、公開済み & 表示 ON のみを返す。並び順は `is_featured DESC, md5(画像id || p_seed), id`（ピン留め → シード付きランダム → タイブレーク）。`p_seed` DEFAULT '' で旧2引数呼び出しも後方互換
...
### 画像ギャラリー
- `post-images` — `extractPostImages` (純粋関数・正規表現のみ・DB 非依存。content 全体を走査し figure(figcaption付き) + 裸 img を出現順マージ、サムネは position=-1)。単体テストあり (`post-images.test.ts`, node --test, 12 ケース)
- `post-images-sync` — `syncPostImages(postId)` (service role で delete→insert, 冪等, best-effort で throw しない)。記事保存 (createPost/updatePost) とミニ記事生成にフック
- `gallery` — 公開側の型 `GalleryImage` + `GALLERY_PAGE_SIZE`(30) + `galleryImageHref` / `galleryImageAlt`
- backfill: `scripts/backfill-post-images.ts` (`npm run backfill:gallery`) で既存記事を一括取込 (実行済み: 96記事/1203枚)
...
- 構造化データ: Article / Breadcrumb / WebSite / Organization / ImageGallery (JSON-LD)
- サイトマップ動的生成 (`/gallery` 含む) / robots.txt (`/api/`・`/admin/` 除外) / canonical URL
- ISR: 記事 1時間 / ホーム 10分。force-dynamic: ギャラリー (訪問ごとランダム並び)。静的: About / Privacy / 404
...
- **RLS**: 全15テーブルで有効
- **SECURITY DEFINER**: リアクション操作 / 閲覧数更新 / AI 利用管理 / ギャラリー取得 (get_gallery_images)
...
| `CRON_SECRET` | 必須 | Vercel Cron keepalive 認証用 (`openssl rand -hex 32`)。Vercel Cron が Bearer で自動付与 |
...
- `?filter=all|visible|hidden|featured` のフィルタタブ + 件数バッジ + グリッド
- 各画像の **表示 ON/OFF** と **ピン留め** トグルのみ (楽観的更新、失敗時ロールバック)。**アップロード/削除はしない** (画像は記事保存時に `syncPostImages` が自動同期)
...
テキスト導線（記事一覧）に対し「写真から記事に入る」導線を追加。仕様の正本は `docs/REQUIREMENTS-gallery.md`。

### 作るもの / 作らないもの
- 作る: 公開 `/gallery` (masonry → 記事直行) / 管理 `/admin/gallery` (表示 ON/OFF + ピン留め) / 記事保存時の自動同期
- 作らない: ライトボックス（タイルは記事へ直行）/ ギャラリー専用のアップロード・削除（画像は記事の持ち物）

### データの流れ
1. **抽出**: `extractPostImages` が記事 content (HTML) + サムネを正規表現で走査し、figure(figcaption付き) + 裸 img を出現順にマージ（サムネは position=-1）。DOM パーサ非使用・DB 非依存の純粋関数
2. **同期**: 記事保存 (createPost/updatePost) とミニ記事生成で `syncPostImages(postId)` を呼び、`post_images` を delete→insert で冪等に更新。公開トグル・本文/サムネ変更も updatePost 経由で追従、記事削除は FK CASCADE で連動
3. **公開取得**: `/gallery` (force-dynamic) と `/api/gallery` (もっと見る) が `get_gallery_images` RPC を呼ぶ。並び順は ピン留め → シード付きランダム。訪問ごとに生成したシードを初回ページと「もっと見る」で共有し、ページングを安定させつつリロードのたびに並びを変える
4. **管理操作**: `/admin/gallery` で `is_visible` / `is_featured` をトグル（mutation は assertAdminAuth + `/admin/gallery`・`/gallery` を revalidate）

### backfill
`scripts/backfill-post-images.ts` (`npm run backfill:gallery`) で既存記事を一括取込（冪等）。実行済み: 96記事 / 1203枚。

---

## 23. Vercel Cron keepalive

Supabase 無料プランは 7日間無アクティビティで停止するため、Vercel Cron で 1 日 1 回 `/api/keepalive` を叩いて実 DB クエリを発生させ停止を防ぐ。

- **エンドポイント**: `/api/keepalive` (`force-dynamic`)。`Authorization: Bearer ${CRON_SECRET}` を検証し、`categories` へ `head:true / count:'exact'` の最軽量クエリ。成功 200 `{ok,count,timestamp}` / 認証失敗 401 / クエリ失敗 500
- **スケジュール**: `vercel.json` の `crons` に `{ "path": "/api/keepalive", "schedule": "0 3 * * *" }`（UTC 3:00 = JST 12:00、1日1回）。Hobby プランの「Cron は1日1回まで」制約にも適合
- **認証**: Vercel Cron が `CRON_SECRET` 環境変数を Bearer トークンとして自動付与
