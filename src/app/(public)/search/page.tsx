...
// Suspense 内で検索を実行し、ヘッダー/検索バーより後からストリーミングする
async function SearchResults({ query, page }: { query: string; page: number }) {
  const { posts, count } = await searchPosts(query, page)
    ...
    <>
      ...
      {posts.length > 0 ? (
        <>
          ...
          {totalPages > 1 && (
            <div className="mt-12">
              <Pagination currentPage={page} totalPages={totalPages} totalItems={count} />
            </div>
          )}
        </>
      ) : (
