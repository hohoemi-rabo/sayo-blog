...
13. **mini_inquiries** - sns_urls text[] (最大5 — 紹介したい SNS 投稿の URL。`message` 自由記述欄は 2026-07-07 に廃止 = migration `20260707120000_drop_mini_inquiries_message.sql`), inquiry_type (event/shop/group/other) + inquiry_type_other, **article_angle (text NULL + CHECK 7値 — 切り口診断 `/request/post/guide` の結果。診断を通らなければ NULL。migration `20260711120000`)**, phone (必須), email, publish_preference (anytime/by_date/in_month) + publish_target_date/month, image_urls text[] (最大2 — **画像と PDF が混在**。チラシは PDF 可。振り分けは `isPdfAttachment()`), consent, status (pending/generating/published/skipped), admin_notes, generated_post_id FK posts (SET NULL)
    - ⚠️ **`sns_urls` と `image_urls` は「どちらか一方が必須」** (2026-07-11)。ターゲットが SNS 投稿者 **+ チラシで宣伝している人**の 2 種類になったため。DB 制約ではなく Zod (`miniInquirySchema` の superRefine) で担保している
14. **long_inquiries** - client_type (individual/organization/group), **desired_plan (必須, monitor/standard/deep/undecided + CHECK, default 'undecided' — LP のプラン表と対応。依頼者の希望であって確定金額ではない)**, individual_name/organization_name/department_name/group_name, contact_person (必須), address (必須), interview_content (必須), publish_preference/interview_preference (free text), phone (必須), email, consent, status (pending/contacted/scheduled/interviewed/writing/published/cancelled), admin_notes, generated_post_id FK posts (SET NULL), scheduled_at, **fee_amount (円 — 紗代さんが後から入れる確定金額。desired_plan とは役割が別)**
