...
>
> **「無料」表現の整理 (2026-07-08)**: LP 本文の「無料」を **18箇所 → 5箇所**に削減。「無料記事」という**名称としての使用は全廃**(すべて「投稿記事」)。「無料」を残したのは訴求として効く箇所のみ = ①ヒーロー主コピー「告知も、自慢も、失敗談も。無料で記事掲載できます。」②「なんで無料で掲載できるの？」見出し ③同セクション本文「投稿記事の掲載を無料にしています。」④`MiniLpForm` 送信欄の注記「掲載は無料です。」。加えて metadata description / OG description に各1回 (SEO 用・本文には表示されない)。**「無料」を増やす方向の編集をするときは、この 4 箇所の役割分担を壊さないこと。**
