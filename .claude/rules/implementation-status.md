...
- `src/app/(lp)/request/mini/actions.ts` - submitMiniInquiry (BotID + レート制限 + ハニーポット + Zod + 画像 + INSERT + メール)
- `src/app/(lp)/request/mini/thanks/page.tsx` - 送信完了画面 (noindex, LP トーン)

> **無料記事 LP 化 (2026-07-06)**: `/request/mini` を `(public)` から独立ルートグループ `(lp)` へ移設し、`index_mini.html` (プロトタイプ) を忠実再現した LP に置き換え。旧 `MiniRequestForm.tsx` は削除、下記の LP セクション参照。
...
### 無料記事 LP (/request/mini) — Complete (2026-07-06)
`/request/mini` の薄いフォーム画面を、`index_mini.html` (紗代さんと相談したプロトタイプ) を忠実再現した LP に置き換え。URL は据え置き (被リンク/canonical/ヘッダー導線をそのまま維持)。
...
- **独立ルートグループ `(lp)`**: サイト共通 Header/Footer を使わないチラシ型ページ。`(public)` の Header/Footer を回避するため別グループに分離
  ...
  - `src/app/(lp)/request/mini/page.tsx` - LP 本体 (Server Component)。hero→about→contents→benefits→reuse→why-free→banner→flow→FAQ→CTA。末尾 `#contact` に `<MiniLpForm>` を埋め込み。FAQPage JSON-LD + canonical + OG。ブランド/フッターは本体 (/ /blog /gallery /request/long /privacy) へ内部リンク (SEO 孤立回避)
  - `src/app/(lp)/request/mini/_components/MiniLpForm.tsx` - LP トーンで再スタイルした Client フォーム。**backend は既存 `submitMiniInquiry` を無改変で再利用**。SNS URL (必須/最大5) + 種別 + 連絡先 + 公開希望 + 画像 + 同意
  - `src/app/sitemap.ts` - `/request/mini` を追加 (monthly, 0.7)
...
- `src/app/(lp)/request/mini/actions.ts` / `src/lib/types.ts` / 管理 (`MiniInquiryDetailDialog.tsx`, `[id]/generate/page.tsx`) から message を除去
- `src/app/(lp)/request/mini/page.tsx` - LP コピー修正 (metadata description + CTA の「URL がなくても大丈夫」→「紹介したい SNS 投稿の URL を送るだけで大丈夫」)。フォームの「伝えたいこと」fieldset 削除 + SNS URL を必須表記に
