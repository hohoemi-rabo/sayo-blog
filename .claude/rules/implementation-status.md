...
- Ticket 41: ミニ記事フロー (公開フォーム /request/post + BotID/Zod/レート制限 + Gmail SMTP 通知 + 管理画面詳細/ステータス/未読バッジ + AI たたき台生成) ✅
- Ticket 42: ロング記事 CRM (公開フォーム /request/interview + 案件管理 7段階ステータス/取材日/金額/記事紐付け + 自動公開連動 + ArticleTypeBadge 公開表示) ✅
...
- 公開: 記事詳細の本文上部に `SummarySlider`。3分割セグメントの背景を **CSS transform でスライド** (framer-motion 不使用)、本文は `key` 差し替え + `animate-slide-in-up`。要約カード下部に本文への橋渡し案内 (「この先は、写真とともに本文で。全文を読みたい方は、このまま下へ。」+ 装飾区切り) を表示 = 要約があるときだけ出る (SummarySlider 内なので未生成記事では非表示)。
- 本文の折りたたみ (「続きを読む」): `src/components/ExpandableArticleBody.tsx` (Client) が `ArticleBody` を children で受け、**全文は SSR で HTML 出力したまま** CSS で冒頭 512px にクランプ + 下部フェード + 「続きを読む」ボタン。JS で本文を後から取得するのではないので SEO は無傷 (全文が初期 HTML にある)。折りたたみ高さより本文が短い記事は `scrollHeight` を実測してボタン/フェードを出さない。TOC は `window.scrollTo` (ハッシュ非使用) のため、隠れた見出しへ飛ぶ事故を防ぐカスタムイベント `EXPAND_EVENT='article-body:expand'` を `TableOfContents` が dispatch → 本文を自動展開してから double-rAF でスクロール。ハッシュ付き直リンクでも自動展開。
...
- `src/components/PostGridSkeleton.tsx` - 一覧ストリーミング中の共通スケルトン (blog/category の Suspense fallback)
- `src/components/ExpandableArticleBody.tsx` - Client, 記事本文を「続きを読む」で展開 (全文は SSR HTML のまま = SEO 安全, CSS クランプ + フェード, 短文は実測して非折りたたみ, TOC 連携で自動展開)
...
- `src/lib/post-queries.ts` - fetchPublishedPosts (公開記事一覧クエリの単一情報源。/blog・/[category]・/api/posts で共用)
...
- `src/app/(public)/about/page.tsx` - 「3 つの記事のかたち」セクション + /request/post, /request/interview CTA

> `/request/interview` 公開フォームは Ticket 42 で実装済み (ComingSoon から差し替え)。
...
- `src/instrumentation-client.ts` - Vercel BotID initBotId (/request/post POST を保護)
...
- `src/app/(lp)/request/post/actions.ts` - submitMiniInquiry (BotID + レート制限 + ハニーポット + Zod + 画像 + INSERT + メール)
- `src/app/(lp)/request/post/thanks/page.tsx` - 送信完了画面 (noindex, LP トーン)

> **無料記事 LP 化 (2026-07-06)**: `/request/post` を `(public)` から独立ルートグループ `(lp)` へ移設し、`index_mini.html` (プロトタイプ) を忠実再現した LP に置き換え。旧 `MiniRequestForm.tsx` は削除、下記の LP セクション参照。
...
### 投稿記事 LP (/request/post, 内部コード名 mini) — Complete (2026-07-06)
> **改名 (2026-07-08) — 公開名「投稿記事」で確定**: 旧「ミニ記事 / 無料記事」。URL を `/request/mini` → `/request/post` へ移設 (next.config.ts の redirects で旧URL・旧thanks を 301)。公開表示 (`ArticleTypeBadge` / About / LP 全体) の「ミニ記事」「無料記事」も「投稿記事」に統一。**DB テーブル `mini_inquiries` / 型 `MiniInquiry` / `article_type='mini'` / 管理画面ラベル / `mini-article-*` 等の内部コード名は mini のまま** (ユーザー不可視のため改名せず、大改修を回避)。以下の記述内の `/request/post` は現行 URL、ファイル名の `MiniLpForm` 等は内部名。
>
> **「無料」表現の整理 (2026-07-08)**: LP 本文の「無料」を **18箇所 → 5箇所**に削減。「無料記事」という**名称としての使用は全廃**(すべて「投稿記事」)。「無料」を残したのは訴求として効く箇所のみ = ①ヒーロー主コピー「告知も、自慢も、失敗談も。無料で記事掲載できます。」②「なんで無料で掲載できるの？」見出し ③同セクション本文「投稿記事の掲載を無料にしています。」④`MiniLpForm` の**送信欄** = 送信ボタン「無料で記事掲載を依頼する」+ 直下の注記「掲載は無料です。費用は一切かかりません。」(2026-07-11 に、最後のひと押しとしてボタン本体にも「無料」を入れた)。加えて metadata description / OG description に各1回 (SEO 用・本文には表示されない)。**「無料」を増やす方向の編集をするときは、この 4 箇所の役割分担を壊さないこと** (④は「送信欄」でひとまとまり。他のセクションに「無料」を撒かない)。

`/request/post` (当初 /request/mini) の薄いフォーム画面を、`index_mini.html` (紗代さんと相談したプロトタイプ) を忠実再現した LP に置き換え。

- **独立ルートグループ `(lp)`**: サイト共通 Header/Footer を使わないチラシ型ページ。`(public)` の Header/Footer を回避するため別グループに分離
  - `src/app/(lp)/layout.tsx` - LP 専用フォント (Cormorant Garamond / Shippori Mincho / Zen Kaku Gothic New) を next/font で self-host + `--font-lp-*` 変数付与 + `.lp-root` ラッパ。lp-mini.css を import
  - `src/app/(lp)/lp-mini.css` - `index_mini.html` の `<style>` を忠実移植し全体を `.lp-root` にスコープ。**このルートでしか読み込まれない** (App Router のルート単位 CSS 分割) ため他ページ非干渉。フォーム用クラス (`.lp-form` 等) も追加
  - `src/app/(lp)/request/post/page.tsx` - LP 本体 (Server Component)。hero→about→contents→benefits→reuse→why-free→banner→flow→FAQ→CTA。末尾 `#contact` に `<MiniLpForm>` を埋め込み。FAQPage JSON-LD + canonical + OG。ブランド/フッターは本体 (/ /blog /gallery /request/interview /privacy) へ内部リンク (SEO 孤立回避)
  - `src/app/(lp)/request/post/_components/MiniLpForm.tsx` - LP トーンで再スタイルした Client フォーム。**backend は既存 `submitMiniInquiry` を無改変で再利用**。SNS URL (必須/最大5) + 種別 + 連絡先 + 公開希望 + 画像 + 同意
  - `src/app/sitemap.ts` - `/request/post` を追加 (monthly, 0.7)

#### 切り口診断 `/request/post/guide` (2026-07-11)
プロトタイプ `index_SNS5web_2.html` を移植。**LP ではなく `/request/post` の下位ページ** = 自前のフォームを持たず、フォームへ送るための補助コンテンツ。URL 階層で親子を表現し、`/request/post/layout.tsx` を継承するので **`.lp-post` スコープと CSS をそのまま共有**（新しい CSS スコープを作らない）。

- **診断の中身**: 「業種」ではなく「どの魅力を前に出したいか」で分岐 (質問5・結果7型)。データは `src/lib/article-angles.ts` に集約 = 診断 UI (Client) と静的タイプ一覧 (Server) の**単一の情報源**
- ⚠️ **結果のコピーは「〜を書く」ではなく「〜の投稿を選ぶ」トーンで書くこと**。フォームが受け取るのは SNS 投稿の URL かチラシであって、自由記述欄は無い。プロトタイプは「5つの情報を書く」前提で書かれており、そのまま移植するとフォームに着いた人が裏切られる (前身 `index_SNS5web.html` の最大の矛盾点だった)
- **SEO**: 7型の中身 (タイトル・送るとよい5投稿・印象) は **静的 HTML としても描画**。JS の中だけに置くとクロールされず資産にならない。各型に `id` を振ってアンカーリンク可
- **結果の引き継ぎ**: 結果 CTA → `/request/post?angle={key}#contact`。`MiniLpForm` が `useSearchParams` で読み、選んだ型のカード + hidden input を出す。**`/request/post` は静的のままにしたいので `<Suspense>` で包む**（外すとページが dynamic に落ちる）
- **DB**: migration `20260711120000_add_mini_inquiries_article_angle.sql` — `mini_inquiries.article_angle` (text NULL + CHECK 7値)。診断を通らない人が大半なので NULL 許容。本番適用済み。管理の詳細ダイアログ・記事化画面・通知メールに表示
- **結果からの出口は 2 つ**: 「この型で投稿記事を送る」(主導線) と「ほかの型と見くらべる」(該当カードへスクロール + `.is-current` で 2.6 秒ハイライト)。結果パネルと型カードは中身が同じなので、**単に飛ばすと同じ文章を 2 度読ませる** → 「見くらべる」ための移動だと分かるよう着地先を光らせている。やり直し系 (前の質問へ / もう一度) は主導線と押し間違えないよう区切り線の下のテキストリンクに落とす
- **`/request/post/guide` に投稿フォームは置かない**。理由は frontend.md「フォームは 1 商品につき 1 URL」参照 (カニバリ / 診断の迂回 / BotID・レート制限の二重管理)
- 「無料記事」というプロトタイプの名称はすべて「投稿記事」に置換 (命名ルール)
- ファイル: `src/lib/article-angles.ts` / `(lp)/request/post/guide/{page.tsx,_components/AngleDiagnosis.tsx}` / `lp-post.css` に `.dg-*`(診断) `.type-*`(7型) `.guide-link`(導線) `.lp-angle`(フォーム側の控え) / `sitemap.ts`

#### チラシ対応 — 「SNS URL か チラシ、どちらか一方が必須」(2026-07-11)
LP のターゲットを **①SNS に投稿している人 + ②チラシで宣伝している人** の 2 種類と定義。②はこれまでフォームを通過できなかった (SNS URL が必須だったため) ので、送信条件を作り直した。

- **必須条件**: `sns_urls` (最大5) **か** 添付 (最大2点) の**どちらか一方**。両方あっても可。両方無しだけがエラー
  - `src/lib/inquiry-schema.ts` — `sns_urls` の `.min(1)` を外し、検証専用フィールド `attachment_count` (DB 非保存) を追加。`superRefine` で「URL 0 件 かつ 添付 0 点」を `path: ['attachments']` のエラーにする
  - `src/app/(lp)/request/post/actions.ts` — 添付点数を Zod に渡すため、**ファイル収集を Zod 検証より前に移動**
- **PDF 対応**: `INQUIRY_ATTACHMENT_ACCEPT` = 画像 + `application/pdf` を新設 (公開フォーム用)。既存 `INQUIRY_IMAGE_ACCEPT` (画像のみ) は**管理画面の「追加画像」用に温存** — 追加画像は記事本文へ `<img>` で差し込むため PDF 不可
  - `uploadInquiryImages` に `accept` オプション追加 (既定は画像のみ)。`EXT_BY_MIME` に pdf 追加
  - 画像と PDF は**同じ `image_urls` カラムに混在**する。振り分けは `src/lib/inquiries.ts::isPdfAttachment(url)` (拡張子判定・Server/Client 両用)
  - `mini-article-generator.ts` は PDF を除外してから記事本文へ画像配置 (PDF は紗代さんが読むための資料)
  - 管理表示は `src/components/admin/InquiryAttachments.tsx` に共通化 (PDF はファイルチップ, 画像はサムネ)。詳細ダイアログと記事化画面で共用
- **フォーム UI 刷新** (`MiniLpForm.tsx`): 「送るもの」fieldset に **SNS URL ボックス / 「または」区切り / チラシ・写真ボックス**を縦積み。連絡先を 2 カラム化、同意の上に写真の使用範囲の説明を明示。並び順は 送るもの → 種別 → 公開希望 → 連絡先 → 同意
- **LP コピー**: `#send` セクション (「送るのは、どちらか一方で大丈夫です。」+ SNS/チラシ の 2 カード + 「または」バッジ) を hero 直後に新設。ヒーロー subcopy・CTA・metadata/OG・FAQ (「SNSをやっていなくても送れますか？」「記事に載せる写真はどうなりますか？」) を追加・更新
- CSS 追加 (`lp-post.css`): `.send-grid` / `.send-or` (LP 本体) と `.lp-sendbox` / `.lp-or` / `.lp-thumb-file` / `.lp-field2` / `.lp-usage` (フォーム)
- **DB マイグレーションなし** (`image_urls` をそのまま使う)

#### 「伝えたいこと」自由記述欄の廃止 (2026-07-07)
ターゲットを「SNS に投稿している人」= 紹介したい SNS 投稿の URL を送る人に絞る方針変更。一度追加した自由記述欄 (message) を撤去し、SNS URL を必須に戻した。
- `supabase/migrations/20260707120000_drop_mini_inquiries_message.sql` - `mini_inquiries.message` を DROP (テーブル 0 行のため無損失。将来必要なら nullable 再追加で戻せる)。本番適用済み
- `src/lib/inquiry-schema.ts` - `message` フィールド + `MINI_MESSAGE_MAX` 削除、`sns_urls` を **min 1**(最大5)に、「URL か本文の一方必須」の superRefine 撤去
- `src/app/(lp)/request/post/actions.ts` / `src/lib/types.ts` / 管理 (`MiniInquiryDetailDialog.tsx`, `[id]/generate/page.tsx`) から message を除去
- `src/app/(lp)/request/post/page.tsx` - LP コピー修正 (metadata description + CTA の「URL がなくても大丈夫」→「紹介したい SNS 投稿の URL を送るだけで大丈夫」)。フォームの「伝えたいこと」fieldset 削除 + SNS URL を必須表記に
- 記事化フロー (`mini-article-generator` = 本文貼付ベース) は URL 起点のまま変更なし
- **種別「その他」= 近所話・雑談の受け皿** (2026-07-07): 種別の下に「当てはまる種別が無ければ『その他』を選んでご自由にお書きください」のガイドを追加。その他選択時の補足欄 (`inquiry_type_other`) を 100字 input → **200字 textarea** に拡張 (schema の max も 200 に)。SNS URL 必須は維持 (近所話も「その投稿を SNS に上げている＝紹介 URL がある」前提)

### 取材記事 LP (/request/interview, 内部コード名 long) — Complete (2026-07-08)
> **公開名「取材記事」で確定** (旧「ロング記事」)。`index_long.html` (プロトタイプ) 忠実再現の独立 LP。
> `(public)` → `(lp)` へ移設。
>
> **URL 改名 (2026-07-08d)**: `/request/long` → **`/request/interview`** (取材 = interview)。
> 旧 URL・旧 thanks は next.config.ts の redirects で **301**。投稿記事 (`/request/mini` → `/request/post`) と同じ方針で、
> **公開 URL と表示名だけを変え、内部コード名 (`long_inquiries` / `LongInquiry` / `article_type='long'` /
> `LongLpForm` / 管理画面ラベル等) は `long` のまま**。以下のファイル名の `Long*` は内部名。

- `src/app/(lp)/request/interview/layout.tsx` - `lp-long.css` を読み `<div className="lp-long">` でラップ
- `src/app/(lp)/lp-long.css` - `index_long.html` の `<style>` を `.lp-long` にスコープ移植 + フォーム部品追加。
  ⚠️ **プロトタイプはテーマ4層の重ね書き** (`:root` × 4) で完成形はカスケード結果 → **層を統合せず定義順を保持**。
  最終パレット: `--magenta:#C08B8B` / `--coral:#E7A1A1` / `--blush:#F6E6E2` (くすんだローズ)
- `src/app/(lp)/request/interview/page.tsx` - LP 本体。hero→about→benefits→**plans(3プラン)**→recommend→prepare→flow→banner→FAQ(8問)→CTA。
  末尾 `#contact` に `<LongLpForm>`。FAQPage JSON-LD + canonical + OG。ヘッダー/フッターからサイト本体へ内部リンク (SEO 孤立回避)
- `src/app/(lp)/request/interview/_components/LongLpForm.tsx` - LP トーンの Client フォーム。**backend は既存 `submitLongInquiry` を再利用**。
  希望プラン (カード選択) + 種別で必須項目を出し分け + 取材内容 (文字数カウンタ) + 連絡先 + 希望 + 同意。旧 `LongRequestForm.tsx` は削除
- `src/app/(lp)/request/interview/thanks/page.tsx` - 完了画面 (noindex, LP トーン)
- `src/app/sitemap.ts` - `/request/interview` を追加 (monthly, 0.7)
- `src/app/(public)/about/page.tsx` - 「ロング記事」→「取材記事」に統一
- **削除**: `src/app/(public)/request/` 配下 (旧フォーム + 未使用の `ComingSoon.tsx`)

#### 希望プラン (desired_plan) — 2026-07-08 追加
LP で 3 プランを明示したため、依頼時点の希望を控える。管理側の `fee_amount` (確定金額) とは役割が別。
- `supabase/migrations/20260708120000_add_long_inquiries_desired_plan.sql` - `long_inquiries.desired_plan`
  (monitor/standard/deep/undecided + CHECK, default 'undecided')。テーブル 0 行のため無損失。本番適用済み
- `src/lib/types.ts` - `LongInquiryPlan` 型 + `LongInquiry.desired_plan`
- `src/lib/inquiries.ts` - `LONG_PLAN_LABELS` (管理表示用) + `LONG_PLAN_OPTIONS` (フォーム選択肢: label/price/hint)。**LP のプラン表と必ず対応させること**
- `src/lib/inquiry-schema.ts` - `LONG_PLANS` + longInquirySchema に `desired_plan` (必須 enum)
- `src/app/(lp)/request/interview/actions.ts` - 読取・INSERT・通知メール要約に反映
- `src/app/(admin)/admin/inquiries/_components/LongInquiryDetailDialog.tsx` - 「希望プラン」行を表示

### Phase 4: ロング記事 CRM (Ticket 42)
> 公開フォームは上記 LP に置き換え済み。以下は CRM (管理側) の記録。

- `src/lib/inquiry-schema.ts` - longInquirySchema 追加 (種別ごと superRefine + 取材内容 200〜2000字)
- `src/app/(lp)/request/interview/actions.ts` - submitLongInquiry (BotID + レート制限 + ハニーポット + Zod + INSERT + メール)
...
- `src/instrumentation-client.ts` - BotID protect に /request/interview POST を追加
