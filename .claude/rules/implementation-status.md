...
- `src/app/(lp)/request/mini/actions.ts` - submitMiniInquiry (BotID + レート制限 + ハニーポット + Zod + 画像 + message + INSERT + メール)
...
- **独立ルートグループ `(lp)`**: サイト共通 Header/Footer を使わないチラシ型ページ。`(public)` の Header/Footer を回避するため別グループに分離
  ...
  - `src/app/(lp)/request/mini/_components/MiniLpForm.tsx` - LP トーンで再スタイルした Client フォーム。**backend は既存 `submitMiniInquiry` を無改変で再利用**。本文欄「伝えたいこと」+ SNS URL (任意) + 種別 + 連絡先 + 公開希望 + 画像 + 同意
- **SNS URL 任意化 + 本文欄追加** (LP が「宣伝だけでなくご近所のどうでもいい情報も歓迎」= URL が無い情報を許容するため):
  - `supabase/migrations/20260706120000_add_mini_inquiries_message.sql` - `mini_inquiries.message` (text NULL) 追加。本番適用済み
  - `src/lib/inquiry-schema.ts` - `sns_urls` を 0〜5件に緩和 (旧 min 1) + `message` (<=2000, `MINI_MESSAGE_MAX`) 追加 + superRefine で **URL か本文の一方必須**
  - `src/app/(lp)/request/mini/actions.ts` - message を読取・INSERT・通知メール要約に反映
  - `src/lib/types.ts` - `MiniInquiry.message`
  - 管理: `MiniInquiryDetailDialog.tsx` と `[id]/generate/page.tsx` に「伝えたいこと」表示 (記事化の参考)
  - `src/app/sitemap.ts` - `/request/mini` を追加 (monthly, 0.7)
- 記事化フロー (`mini-article-generator` = 本文貼付ベース) と一覧は URL 0件でも既存のまま動く
