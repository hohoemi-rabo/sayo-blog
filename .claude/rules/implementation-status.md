...
- `src/app/(public)/about/page.tsx` - 「3 つの記事のかたち」セクション + /request/post, /request/long CTA
...
- **独立ルートグループ `(lp)`**: サイト共通 Header/Footer を使わないチラシ型ページ。`(public)` の Header/Footer を回避するため別グループに分離
  ...
  - `src/app/(lp)/request/post/page.tsx` - LP 本体 (Server Component)。hero→about→contents→benefits→reuse→why-free→banner→flow→FAQ→CTA。末尾 `#contact` に `<MiniLpForm>` を埋め込み。FAQPage JSON-LD + canonical + OG。ブランド/フッターは本体 (/ /blog /gallery /request/long /privacy) へ内部リンク (SEO 孤立回避)
...
### 取材記事 LP (/request/long, 内部コード名 long) — Complete (2026-07-08)
> **公開名「取材記事」で確定** (旧「ロング記事」)。`index_long.html` (プロトタイプ) 忠実再現の独立 LP。
> `(public)` → `(lp)` へ移設したが **URL は据え置き** (`/request/long`) のためリダイレクト不要。

- `src/app/(lp)/request/long/layout.tsx` - `lp-long.css` を読み `<div className="lp-long">` でラップ
...
- `src/app/(lp)/request/long/page.tsx` - LP 本体。hero→about→benefits→**plans(3プラン)**→recommend→prepare→flow→banner→FAQ(8問)→CTA。
  末尾 `#contact` に `<LongLpForm>`。FAQPage JSON-LD + canonical + OG。ヘッダー/フッターからサイト本体へ内部リンク (SEO 孤立回避)
- `src/app/(lp)/request/long/_components/LongLpForm.tsx` - LP トーンの Client フォーム。**backend は既存 `submitLongInquiry` を再利用**。
  希望プラン (カード選択) + 種別で必須項目を出し分け + 取材内容 (文字数カウンタ) + 連絡先 + 希望 + 同意。旧 `LongRequestForm.tsx` は削除
- `src/app/(lp)/request/long/thanks/page.tsx` - 完了画面 (noindex, LP トーン)
- `src/app/sitemap.ts` - `/request/long` を追加 (monthly, 0.7)
...
- `src/app/(lp)/request/long/actions.ts` - 読取・INSERT・通知メール要約に反映
...
- `src/app/(lp)/request/long/actions.ts` - submitLongInquiry (BotID + レート制限 + ハニーポット + Zod + INSERT + メール)
