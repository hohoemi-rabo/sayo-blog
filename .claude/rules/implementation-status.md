...
> **改名 (2026-07-08)**: 公開名を「ミニ記事」→「投稿記事」に変更し、URL を `/request/mini` → `/request/post` へ移設 (next.config.ts の redirects で旧URL・旧thanks を 301)。公開表示 (`ArticleTypeBadge` / About) の「ミニ記事」も「投稿記事」に。**DB テーブル `mini_inquiries` / 型 `MiniInquiry` / `article_type='mini'` / 管理画面ラベル / `mini-article-*` 等の内部コード名は mini のまま** (ユーザー不可視のため改名せず、大改修を回避)。LP 本文の「無料」多用 (18箇所) の整理は別途保留。以下の記述内の `/request/post` は現行 URL、ファイル名の `MiniLpForm` 等は内部名。
